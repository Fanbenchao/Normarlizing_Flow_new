...
class AffineCouplingCondYG(tfb.Bijector):
    ...
    def __init__(
            ...
            validate_args=False,
            name="real_nvp"):
        super(AffineCouplingCondYG,self).__init__(
    ...
    def _forward(self, x, yy, nlf0=None, nlf1=None, iso=None, cam=None):
        ...
        if yy.shape[1] == 2 * x.shape[1]:  # needs squeezing
            yy = squeeze2d(yy, 2)
        shift, log_scale = self._shift_and_log_scale_fn(yy, iso)
    ...
    def _inverse(self, y, yy, nlf0=None, nlf1=None, iso=None, cam=None):
        shift, log_scale = self._shift_and_log_scale_fn(yy, iso)
        tf.summary.histogram('cYG shift', shift)
        tf.summary.histogram('cYG log_scale', log_scale)
        log_scale = self.scale * tf.tanh(log_scale)
        x = y  # y[:, :, :, self.ic // 2:]
        if log_scale is not None:
            x *= tf.exp(log_scale)
    ...
    def _forward_log_det_jacobian(self, x, yy, nlf0=None, nlf1=None, iso=None, cam=None):
        if self._last_layer:
            x = tf.reshape(x, (-1, self.i0, self.i1, self.ic))
            yy = tf.reshape(yy, (-1, self.i0, self.i1, self.ic))
        _, log_scale = self._shift_and_log_scale_fn(yy, iso)
        log_scale = self.scale * tf.tanh(log_scale)
