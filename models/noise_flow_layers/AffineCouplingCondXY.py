...
class AffineCouplingCondXY(tfb.Bijector):
    ...
    def __init__(
            ...
            validate_args=False,
            name="real_nvp"):
        super(AffineCouplingCondXY,self).__init__(
    ...
    def _forward(self, x, yy, nlf0=None, nlf1=None, iso=None, cam=None):
        ...
        if x.shape != yy.shape:
            yy = tf.reshape(yy, x.shape[:])
        x0 = x[:, :, :, :self.ic // 2]
        x1 = x[:, :, :, self.ic // 2:]
        x0yy = tf.concat([x0, yy], axis=-1)
        shift, log_scale = self._shift_and_log_scale_fn(x0yy)
    ...
    def _inverse(self, y, yy, nlf0=None, nlf1=None, iso=None, cam=None):
        ...
        shift, log_scale = self._shift_and_log_scale_fn(y0yy)
    ...
    def _forward_log_det_jacobian(self, x, yy, nlf0=None, nlf1=None, iso=None, cam=None):
        ...
        x0 = x[:, :, :, :self.ic // 2]
        x0yy = tf.concat([x0, yy], axis=-1)
        _, log_scale = self._shift_and_log_scale_fn(x0yy)
        log_scale = self.scale * tf.tanh(log_scale)
    ...
    def _inverse_log_det_jacobian(self, z, yy, nlf0=None, nlf1=None, iso=None, cam=None):
        ...
        _, log_scale = self._shift_and_log_scale_fn(z0yy)
        tf.summary.histogram('cXY log_scale', log_scale)
        log_scale = self.scale * tf.tanh(log_scale)
        if log_scale is None:
