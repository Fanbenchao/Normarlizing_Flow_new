...
from models.layers import AffineCoupling,Conv2d1x1,real_nvp_conv_template,real_nvp_conv_template_iso,conv2d_zeros
from models.utils import int_shape,squeeze2d,unsqueeze2d
from models.noise_flow_layers.AffineCouplingCondXY import AffineCouplingCondXY
from models.noise_flow_layers.AffineCouplingCondXYG import AffineCouplingCondXYG
from models.noise_flow_layers.AffineCouplingCondY import AffineCouplingCondY
from models.noise_flow_layers.AffineCouplingCondYG import AffineCouplingCondYG
from models.noise_flow_layers.AffineCouplingGain import AffineCouplingGain
from models.noise_flow_layers.AffineCouplingGainEx1 import AffineCouplingGainEx1
from models.noise_flow_layers.AffineCouplingGainEx2 import AffineCouplingGainEx2
from models.noise_flow_layers.AffineCouplingGainEx3 import AffineCouplingGainEx3
from models.noise_flow_layers.AffineCouplingGainEx4 import AffineCouplingGainEx4
from models.noise_flow_layers.AffineCouplingSdn import AffineCouplingSdn
from models.noise_flow_layers.AffineCouplingSdnEx1 import AffineCouplingSdnEx1
from models.noise_flow_layers.AffineCouplingSdnEx2 import AffineCouplingSdnEx2
from models.noise_flow_layers.AffineCouplingSdnEx3 import AffineCouplingSdnEx3
from models.noise_flow_layers.AffineCouplingSdnEx4 import AffineCouplingSdnEx4
from models.noise_flow_layers.AffineCouplingSdnEx5 import AffineCouplingSdnEx5
from models.noise_flow_layers.AffineCouplingSdnEx6 import AffineCouplingSdnEx6
from models.noise_flow_layers.AffineCouplingSdnGain import AffineCouplingSdnGain
from models.noise_flow_layers.AffineCouplingCamSdn import AffineCouplingCamSdn
from models.noise_flow_layers.AffineCouplingFitSdnGain2 import AffineCouplingFitSdnGain2
...
class NoiseFlow(object):
    ...
    def __init__(self, x_shape, is_training, hps):
    ...
    def noise_flow_arch(self, name, x_shape, flow_permutation, arch):
        ...
        with tf.compat.v1.variable_scope(name):
            for i, lyr in enumerate(arch_lyrs):
                with tf.compat.v1.variable_scope('bijector{}'.format(i)):
                    is_last_layer = False
                    ...
                    if lyr == 'unc':
                        ...
                        bijectors.append(
                            AffineCouplingSdn(
                                ...
                                names = ['b1_'+str(i),'sigmoid_b1_'+str(i),'b2_'+str(i),'sigmoid_b2_'+str(i)]
                            )
                        )
                    elif lyr == 'sdn1':
                        ...
                        bijectors.append(
                            AffineCouplingGainEx1(
                                ...
                                shift_and_log_scale_fn=None,
                                param_inits=self.hps.param_inits,
                                names = ['shutter_params_'+str(i),'g1_'+str(i),'g2_'+str(i)]
                            )
                        )
                    elif lyr == 'gain2':
    ...
    def revnet2d(self, name, x_shape, flow_permutation):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            if self.hps.append_sdn2:
                with tf.compat.v1.variable_scope('bijector_sdn2'):
            ...
            if self.hps.append_sdn_first:
                with tf.compat.v1.variable_scope('bijector_sdn'):
            ...
            if self.hps.append_cY:
                with tf.compat.v1.variable_scope('bijector_cy'):
            ...
            if self.hps.append_sdn:
                with tf.compat.v1.variable_scope('bijector{}'.format(self.depth)):
                    print('|-AffineCouplingFitSDN')
                    bijectors.append(
                        AffineCouplingSdnGain(
    ...
    def inverse(self, x, objective, yy=None, nlf0=None, nlf1=None, iso=None, shutter=None):
        z = x
            ...
            for bijector in self.model[i]:
                if type(bijector) in [AffineCouplingCondY, AffineCouplingCondXY, AffineCouplingFitSdnGain2,
                    ...
                    try:
                        ...
                        log_abs_det_J_inv = bijector._inverse_log_det_jacobian(z, yy, nlf0, nlf1, iso, shutter)
                else:
                    try:
                        z, log_abs_det_J_inv = \
    ...
    def forward(self, z, eps_std, yy=None, nlf0=None, nlf1=None, iso=None, shutter=None):
        x = z
        for i in reversed(range(self.n_levels)):
            ...
            for bijector in reversed(self.model[i]):
                if type(bijector) in [AffineCouplingCondY, AffineCouplingCondXY, AffineCouplingFitSdnGain2,
                                      ...
                                      AffineCouplingSdnEx4, AffineCouplingGainEx4, AffineCouplingSdnEx5,
                                      AffineCouplingSdnEx6]:
                    x = bijector._forward(x, yy, nlf0, nlf1, iso, shutter)
                else:
                    x = bijector._forward(x)
            x = unsqueeze2d(x, self.hps.squeeze_factor, self.hps.squeeze_type)
        return x

    def sample(self, y, eps_std=None, yy=None, nlf0=None, nlf1=None, iso=None, shutter=None):
        """Sampling function"""
        # y_onehot = tf.cast(tf.one_hot(tf.cast(y, 'int32'), self.hps.n_y, 1, 0), 'float32')  # not used for sidd problem
        with tf.compat.v1.variable_scope('model', reuse=True):
            _, sample = self.prior("prior", y) #从指定均值与方差的正太分布中采样与y相同尺寸的随机张量
            z = sample(eps_std)
            x = self.forward(z, eps_std, yy, nlf0, nlf1, iso, shutter)
        return x

    def _loss(self, x, y, nlf0=None, nlf1=None, iso=None, shutter=None, reuse=False):
        with tf.compat.v1.variable_scope('model', reuse=reuse):
            ...
            if self.hps.sidd_cond is not None and self.hps.sidd_cond != 'uncond':
                z, objective = self.inverse(x, objective, yy=y, nlf0=nlf0, nlf1=nlf1, iso=iso, shutter=shutter)
            else:
    ...
    def loss(self, x, y, nlf0=None, nlf1=None, iso=None, shutter=None, reuse=False):
        nll, sd_z = self._loss(x=x, y=y, nlf0=nlf0, nlf1=nlf1, iso=iso, shutter=shutter, reuse=reuse)  # returns NLL
...
def gaussian_diag(mean, logsd):
    ...
    o.mean = mean
    o.logsd = logsd
    o.eps = tf.compat.v1.random_normal(tf.shape(mean))
...
def get_optimizer(hps, lr, loss_val):
    ...
    if hps.sidd_cond != 'condSDN':
        if hps.optim == 'adam':
            train_op = tf.compat.v1.train.AdamOptimizer(learning_rate=lr,
                                              beta1=0.9,
                                              beta2=0.999,
                                              epsilon=1e-08).minimize(loss_val)
        elif hps.optim == 'sgd':
            train_op = tf.compat.v1.train.MomentumOptimizer(lr, 0.9).minimize(loss_val)
